<template >
  <div class="tool-bar" :class="menuStore.layerIsShow ? '' : 'tool-transform'">
    ...
    >
      ...
      <div class="tool-main">
        <div>
          <el-button type="primary" @click="handlePic">开始分析</el-button>
  ...
  <div class="skyResult">
    <el-dialog v-model="skylineVisible" title="Tips" width="100%">
...
import SkyLineDrawer from "./skyclass/SkyLineDrawer";

const { menuStore, viewStore, mapStore } = useStore();

const locateVisible = ref(false);

let skylineVisible = ref(false);
let draw:any = null;
const closeLocate = () => {
  if (groupLayer.value) {
...
const handlePic = () => {
  ...
  let abc = sceneView.map.findLayerById("skylayer");
  if (!abc) {
    let renderLayer = new GraphicsLayer({
      id: "skylayer",
      elevationInfo: {
        mode: "on-the-ground",
      },
    });
    sceneView.map.layers.add(renderLayer);
  }

  sceneView.goTo(camera).then(() => {
    // console.log('123');
    draw = new SkyLineDrawer(sceneView, environment);
    draw.execute();
    viewStore.handleSkyLoading(true);
    // executeDraw();
  });
};
let environment = {
